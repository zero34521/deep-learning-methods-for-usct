...
for fold in range(1,6):
    ...
    for i, data in enumerate(test_loader):
        ...
        plt.figure()
        plt.imshow(y2_hat_test[0, 0, :, :], vmin=-1, vmax=1)
        plt.colorbar()
        # savename2 = folder_path+'/fold'+str(fold)+'/test_results/sim/' + 'test_direct_' + str(i)
        savename2 = r'/home/user/yanguo/pycharm_connection/reconstruction_USCT' \
                    r'/results_pic_ren/results_direct_post_128/' + 'test_post_128_' + str(i)
        plt.savefig(savename2)
        plt.close()
        plt.figure()
        plt.imshow(y2_hat_test2[0, 0, :, :], vmin=-1, vmax=1)
        plt.colorbar()
        # savename2 = folder_path+'/fold'+str(fold)+'/test_results/sim/' + 'test_post_' + str(i)
        savename2 = r'/home/user/yanguo/pycharm_connection/reconstruction_USCT' \
                    r'/results_pic_ren/results_direct_post_256/' + 'test_post_256_' + str(i)
        plt.savefig(savename2)
        plt.close()
        '''把预测结果保存在模型'''
