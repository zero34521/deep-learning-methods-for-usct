...
from My_models.test_models import Postprocessnet, UNet_origin, DirectNet2
...
for fold in range(1,6):
    ...
    Y1_train = np.load(load_data_path3 + '/slowness_mix_label_train_aug_256.npy')
    Y2_train = np.load(load_data_path3 + '/slowness_mix_label_train_aug_256.npy')

    X_val = np.load(load_data_path2 + '/TTDMs_mix_fold' + str(fold) + '_test.npy')
    Y1_val = np.load(load_data_path4 + '/slowness_mix_label_fold' + str(fold) + '_test_256.npy')
    Y2_val = np.load(load_data_path4 + '/slowness_mix_label_fold' + str(fold) + '_test_256.npy')
    X_train = np.transpose(X_train)  # 用hdf5导入的文件，还得转置一下才行
    Y1_train = np.transpose(Y1_train)  # 用hdf5导入的文件，还得转置一下才行
    Y2_train = np.transpose(Y2_train)  # 用hdf5导入的文件，还得转置一下才行

    '''加载吉洪诺夫矩阵作为初值'''
    T_inv = np.load(r'/home/user/yanguo/pycharm_connection/reconstruction_USCT/data/FC_matrix/a_full_128to256_gamma_5.npy')
    T_inv = np.transpose(T_inv)
    '''量级变换'''
    ...
    direct_net=DirectNet2(128, 256)
    folder_path = '/home/hdd/yanguo/results_cv5'
    ...
    message = '-------------------------------------------------------------------------------------------------------'
    with open(r'/home/hdd/yanguo/result_ren/results_cv5/fold'+str(fold)+'/loss_results/log.txt', 'a') as log_f:
       log_f.write('%s\n' % message)
       log_f.write('%s\n' % message_batch_size)
       log_f.write('%s\n' % device_message)
    #学习率初始值设定
    ...
    for epoch in range(1,300):
       ...
       if epoch % 2 == 0:
          ...
          plt.colorbar()
          savename1 = r'/home/hdd/yanguo/result_ren/results_cv5/fold'+str(fold)+'/train_results/'+'train_direct_epoch' + str(epoch)
          plt.savefig(savename1)
          plt.close()
          plt.figure()
          plt.imshow(compare_result_train2,vmin=-1,vmax=1)
          plt.colorbar()
          savename2 = r'/home/hdd/yanguo/result_ren/results_cv5/fold'+str(fold)+'/train_results/'+'train_post_epoch' + str(epoch)
       ...
       if epoch % 2 == 0:
          ...
          plt.colorbar()
          savename2 = r'/home/hdd/yanguo/result_ren/results_cv5/fold'+str(fold)+'/train_results/'+'val_direct_epoch' + str(epoch)
          plt.savefig(savename2)
          plt.close()
          plt.figure()
          plt.imshow(compare_result_val2,vmin=-1,vmax=1)
          plt.colorbar()
          savename2 = r'/home/hdd/yanguo/result_ren/results_cv5/fold'+str(fold)+'/train_results/'+'val_post_epoch' + str(epoch)
          plt.savefig(savename2)
          plt.close()
       ...
       #    save_path = r'/home/hdd/yanguo/results_cv1/fold'+str(fold)+'/model_results/'
       ...
       if val_loss_best_post > val_loss_aver_post:
          ...
          with open(r'/home/hdd/yanguo/result_ren/results_cv5/fold'+str(fold)+'/loss_results/log.txt', 'a') as log_f:
             log_f.write('%s\n' % message_update)
          val_loss_best_post = val_loss_aver_post
          save_path = r'/home/hdd/yanguo/result_ren/results_cv5/fold'+str(fold)+'/model_results/'
          save_model_name = 'Post_of_model_best' # option:u-net twolayersnet resnet
          save_path_and_model_name = save_path + save_model_name
          '''方式一'''
          # torch.save(recon_net.state_dict(), save_path_and_model_name)
    ...
    with open(r'/home/hdd/yanguo/result_ren/results_cv5/fold' + str(fold) + '/loss_results/log.txt', 'a') as log_f:
        log_f.write('%s\n' % message_update)
    '''保存学习率曲线数据'''
    np.save(r'/home/hdd/yanguo/result_ren/results_cv5/fold'+str(fold)+'/loss_results/train_loss_post.npy', train_loss_post)
    np.save(r'/home/hdd/yanguo/result_ren/results_cv5/fold'+str(fold)+'/loss_results/val_loss_save_post.npy',val_loss_save_post)
    np.save(r'/home/hdd/yanguo/result_ren/results_cv5/fold'+str(fold)+'/loss_results/train_loss_direct.npy', train_loss_direct)
    np.save(r'/home/hdd/yanguo/result_ren/results_cv5/fold'+str(fold)+'/loss_results/val_loss_save_direct.npy',val_loss_save_direct)
    #############最终模型的保存############
    '''-----------后处理模型保存-----------'''
    '''方式一'''
    # save_path=r'/home/hdd/yanguo/results_cv1/fold'+str(fold)+'/model_results/'
    ...
    pic_savename1 = '/home/hdd/yanguo/result_ren/results_cv5/fold'+str(fold)+'/loss_results/train_val_loss_post.png'
    ...
    pic_savename1 = '/home/hdd/yanguo/results_cv1/fold'+str(fold)+'/loss_results/train_val_loss_direct.png'
    plt.savefig(pic_savename1)
    plt.show()
    plt.close()
